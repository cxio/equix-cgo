...
| `go.mod` | 已有，保持 `module github.com/cxio/equix` / `go 1.27.0` |

`internal/native` 不得 import `github.com/cxio/equix`（循环）。native 的 Verify 返回与 C `equix_result` 相同的 `int`（0=OK, 1=CHALLENGE, 2=ORDER, 3=PARTIAL_SUM, 4=FINAL_SUM）；公开包映射到哨兵错误。
...
- Modify: `go.mod`（若尚未纳入 git，一并加入；内容已是 `module github.com/cxio/equix` / `go 1.27.0`）
